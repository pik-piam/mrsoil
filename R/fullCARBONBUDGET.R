...
fullCARBONBUDGET <- function(rev = 0.1, dev = "") {
  ...
  if (grepl("histManagement", dev)) {
    ...
    calcOutput("SoilCarbonSpinup", cfg_default = cfgInit, aggregate = FALSE, file = "SoilCarbonSpinup.rds")
    ...
    ### point data
    calcOutput("SOCPointData", aggregate = FALSE, file = "calcSOCPointData.rds")
